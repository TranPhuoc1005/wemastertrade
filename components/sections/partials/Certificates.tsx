...
interface CertificatesProps extends LangProps {
  dict?: Record<string, string>;
}

export default function Certificates({lang, dict}: CertificatesProps) {
  const t = dict || {};
  return (
    <section className="certificates">
        <div className="container">
            <div className="certificates__frame">
                <div className="title">
                    <p className="title__sub">{t.certificates_block_0 || "Certification"}</p>
                    {t.certificates_block_1 ? (
                        <h3 dangerouslySetInnerHTML={{ __html: t.certificates_block_1 }} />
                    ) : (
                        <h3>Awards & <br /><span>Recognition</span></h3>
                    )}
                    <div className="btn"><p><Link href={`/${lang}/certification/`}><span>{t.certificates_block_2 || "Learn more"}</span></Link></p></div>
                </div>
                ...
                <div className="certificates__content">
                    <div className="frame">
                        <div className="desc">
                            ...
                            <p className="txt">{t.certificates_block_3 || "Clients are provided with a simulated account. Their trading activity on this account is replicated in real-time by our exclusive algorithms to our live firm trading account, generating actual cash flow."}</p>
                    ...
                    <div className="btn btn__download center">
                        <p><Link href={`/${lang}/handbook-page/`}><span>{t.certificates_block_4 || "Download Handbook"}</span></Link></p>
                    </div>
                </div>
